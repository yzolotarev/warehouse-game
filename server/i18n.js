...
(function () {
  ...
  const PAIRS = [
    ...
    ["стеллажи · по контекстам, всплывает на пересборке", "racks · by context, resurfaces at the weekly rebuild"],
    ...
    ["Недельная пересборка", "Weekly rebuild"],
    ["итоги + ревизия всех полок · +25 ⭐", "recap + audit of every shelf · +25 ⭐"],
    ...
    ["столп №2: без неё система разваливается за две недели", "pillar #2: without it the system rots in two weeks"],
    ...
    // ── недельная пересборка ──
    ["столп №2: раз в неделю пройтись по всему складу, чтобы он не сгнил", "pillar #2: once a week walk the whole warehouse so it doesn't rot"],
    ["неделя на складе", "the week at the warehouse"],
    ...
    ["следующая пересборка через неделю — склад позовёт сам", "next rebuild in a week — the warehouse will call you"],
